	...
	const set: BaileysEventHandler<"messaging-history.set"> = async ({ messages, isLatest }) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during messages set: ${e instanceof Error ? e.message : "Unknown error"}`,
			);
		}
	};
				...
				for (const message of messages) {
					try {
						...
						emitEvent(
							...
							undefined,
							"error",
							`An error occured during message upsert: ${e instanceof Error ? e.message : "Unknown error"}`,
						);
					}
				}
				break;
		...
		for (const { update, key } of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during message update: ${e instanceof Error ? e.message : "Unknown error"}`,
				);
			}
		}
	};
	...
	const del: BaileysEventHandler<"messages.delete"> = async (item) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during message delete: ${e instanceof Error ? e.message : "Unknown error"}`,
			);
		}
	};
		...
		for (const { key, receipt } of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during message receipt update: ${e instanceof Error ? e.message : "Unknown error"}`,
				);
			}
		}
	};
		...
		for (const { key, reaction } of reactions) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during message reaction update: ${e instanceof Error ? e.message : "Unknown error"}`,
				);
			}
		}
	};
