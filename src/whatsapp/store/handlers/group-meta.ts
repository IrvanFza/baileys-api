	...
	const upsert: BaileysEventHandler<"groups.upsert"> = async (groups) => {
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during groups upsert: ${e instanceof Error ? e.message : "Unknown error"}`,
			);
		}
	};
		...
		for (const update of updates) {
			try {
				...
				emitEvent(
					...
					undefined,
					"error",
					`An error occured during group metadata update: ${e instanceof Error ? e.message : "Unknown error"}`,
				);
			}
		}
	};
	...
	const updateParticipant: BaileysEventHandler<"group-participants.update"> = async ({
		...
		try {
			...
			emitEvent(
				...
				undefined,
				"error",
				`An error occured during group participants update: ${e instanceof Error ? e.message : "Unknown error"}`,
			);
		}
	};
